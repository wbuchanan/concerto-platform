...
Methods.getDocContent = function(html) {
    ...
    //html = "<div><button class='btnFunctionWizard' onclick='Methods.uiWriteFunctionWidget()'>" + dictionary["s682"] + "</button></div><div id='divFunctionDoc'>" + html + "</div>";
    html = "<div id='divFunctionDoc'>" + html + "</div>";
    return html;
}

Methods.uiWriteFunctionWidget = function(func, html) {
    if (html == null)
        html = $("#divFunctionDoc").html();

    var parsedDoc = Methods.getParsedDoc(func, html);
}

Methods.get

Methods.getParsedDoc = function(func, html) {
    var parsedDoc = {};
    var obj = $(html);
...
Methods.uiWriteAutocompleteDoc = function(html) {
    $("#divCodeAutocompleteDoc").html(Methods.getDocContent(html));
    //Methods.iniIconButton(".btnFunctionWizard", "lightbulb");
}
                                ...
                                if (doc == null) {
                                    ...
                                    if (!Methods.documentationLoaderIsWorking) {
                                        ...
                                        $.post("query/r_documentation.php", {
                                            ...
                                            Methods.uiWriteAutocompleteDoc(data.html);
                                            Methods.documentationLoaderIsWorking = false;
                                            $("#selectCodeAutocomplete").change();
                                        }, "json");
                                    }
                                } else {
                                    Methods.uiWriteAutocompleteDoc(doc.html);
                                }
                            });
                            ...
                            $("#selectCodeAutocomplete").keydown(function(e) {
                                ...
                                if (e.ctrlKey && code == 13) {
                                    ...
                                    if (selectedOption.length == 0)
                                        return;
                                    var doc = Methods.getFuncDoc(selectedOption.attr("value"), selectedOption.attr("pack"));
                                    if (doc == null)
                                        return;
                                    Methods.uiWriteFunctionWidget(selectedOption.attr("value"), Methods.getDocContent(doc.html));
